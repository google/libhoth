...
use core::slice;
use libhoth_error::HOSTCMD_BUFFER_OVERFLOW;
use libhoth_error::HOSTCMD_LEN_U16_OVERFLOW;
...
// Re-export this handy type; in case it ever moves out of mutask_types.
pub use libhoth_aligned::U64Align4;
...
impl uDisplay for HostCommand {
    fn fmt<W>(&self, f: &mut ufmt::Formatter<'_, W>) -> Result<(), W::Error>
    where
        W: ufmt::uWrite + ?Sized,
    {
        uwrite!(f, "0x{:04x}", self.0)
    }
}
...
pub trait HostCmdReq: FromBytes + IntoBytes + Immutable + KnownLayout {
    ...
    #[inline(always)]
    fn header_template(data_len: usize) -> Result<RequestHeader, ErrorCode> {
        Ok(RequestHeader {
            ...
            data_len: u16::try_from(data_len).map_err(|_| HOSTCMD_LEN_U16_OVERFLOW)?,
        })
    }
    ...
    #[inline(always)]
    fn header_template_fixed() -> Result<RequestHeader, ErrorCode>
    where
        Self: Sized,
    {
    ...
    ///
    /// `ReqBuilder::data` will be zero-initialized.
    ///
    /// # Compile-time error
    ///
    ...
    fn build_dynamic(
        ...
        num_elems: usize,
    ) -> Result<&mut ReqBuilder<Self>, ErrorCode>
    where
        Self: Pointee<Metadata = usize>,
    {
        ...
        if total_size_aligned > buf.len() {
            return Err(HOSTCMD_BUFFER_OVERFLOW);
        }
    ...
    /// Helper for building a fixed-sized request.
    ///
    /// `buf` is the buffer that will contain the request. Returns a mutable reference to a
    /// `ReqBuilder` that can be used to populate the request and then finalize it. Upon
    /// finalization, all the header fields (including checksum) will be set.
    ///
    /// `ReqBuilder::data` will be zero-initialized.
    ///
    /// # Errors
    ///
    /// Returns [`HOSTCMD_BUFFER_OVERFLOW`] if `buf` is smaller than
    /// `size_of::<ReqBuilder<Self>>()`; this includes the builder's tail
    /// padding, so it can exceed `size_of::<RequestHeader>() + size_of::<Self>()`.
    ///
    /// # Compile-time error
    ///
    /// Causes a compile-time error if `Self` is greater than 4-byte aligned.
    #[inline(always)]
    fn build_fixed(buf: &mut Aligned<A4, [u8]>) -> Result<&mut ReqBuilder<Self>, ErrorCode>
    where
        Self: Sized,
    {
        const {
            assert!(
                align_of::<ReqBuilder<Self>>() <= 4,
                "ReqBuilder<Self> must be at most 4-byte aligned"
            )
        };
        if buf.len() < size_of::<ReqBuilder<Self>>() {
            return Err(HOSTCMD_BUFFER_OVERFLOW);
        }
...
mod request {
    use libhoth_error::IO_GENERIC_ZEROCOPY_CONV_SIZE;
    ...
    impl Request {
        ...
        pub fn ref_from_bytes(r: &Aligned<A4, [u8]>) -> Result<&Self, ErrorCode> {
            let data_len = r
                .len()
                .checked_sub(Self::DATA_OFFSET)
                .ok_or(IO_GENERIC_ZEROCOPY_CONV_SIZE)?;
            Ok(unsafe {
        ...
        pub fn data<T>(&self) -> Result<&T, ErrorCode>
        where
            T: FromBytes + KnownLayout + Immutable,
        {
            T::ref_from_bytes(&self.data_bytes).map_err(|e| -> ErrorCode { e.into() })
        }
        ...
        pub fn data_dynamic<T>(&self) -> Result<&T, ErrorCode>
        where
            T: ?Sized
                ...
                + Immutable,
        {
            ...
            Ok(T::ref_from_prefix_with_elems(&self.data_bytes, num_elems)?.0)
        }
    ...
    impl Request {
        pub fn mut_from_bytes(r: &mut Aligned<A4, [u8]>) -> Result<&mut Self, ErrorCode> {
            let data_len = r
                .len()
                .checked_sub(Self::DATA_OFFSET)
                .ok_or(IO_GENERIC_ZEROCOPY_CONV_SIZE)?;
    ...
    impl Debug for Request {
        fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
            f.debug_struct("Request")
                ...
                .field("data", &HexDisplay(&self.data_bytes))
                .finish()
        }
    }
    ...
    mod test_request {
        ...
        fn test_ref_from_bytes_invalid_input() {
            assert_eq!(
                ...
                Err(IO_GENERIC_ZEROCOPY_CONV_SIZE)
            );
            assert_eq!(
        ...
        fn debug() {
            ...
            assert_eq!(
                format!("{req:?}"),
                "Request { version: 1, checksum: 2, command: HostCommand(3), command_version: 4, reserved: 5, data_len: 6, data: [0x07, 0x08, ] }"
            );
        }
    }
}
...
#[derive(Debug, FromBytes, Immutable, KnownLayout, IntoBytes, Eq, PartialEq, uDebug)]
#[repr(C)]
...
mod response {
    ...
    impl uDebug for Response {
        fn fmt<W: uWrite + ?Sized>(&self, f: &mut ufmt::Formatter<'_, W>) -> Result<(), W::Error> {
            f.debug_struct("Response")?
                .field("header", &self.header)?
                // Manually handle the unsized field. Let's just print the first 8 bytes.
                .field(
                    "data_bytes (prefix)",
                    &self.data_bytes.get(..8).unwrap_or(&self.data_bytes),
                )?
                .finish()
        }
    }
    impl Response {
        ...
        pub fn mut_from_prefix_with_elems(
            ...
            if data_len > u16::MAX.into() {
                return Err(IO_GENERIC_ZEROCOPY_CONV_SIZE);
            }
        ...
        pub fn respond_ok<'res, T: HostCmdResp>(
            ...
            data: &T,
        ) -> Result<&'res ResponseFinalized, ErrorCode> {
        ...
        pub fn ref_from_bytes(bytes: &Aligned<A4, [u8]>) -> Result<&Self, ErrorCode> {
            let data_len = bytes
                .len()
                .checked_sub(Self::DATA_OFFSET)
                .ok_or(IO_GENERIC_ZEROCOPY_CONV_SIZE)?;
        ...
        pub fn mut_from_bytes(bytes: &mut Aligned<A4, [u8]>) -> Result<&mut Self, ErrorCode> {
            let data_len = bytes
                .len()
                .checked_sub(Self::DATA_OFFSET)
                .ok_or(IO_GENERIC_ZEROCOPY_CONV_SIZE)?;
    ...
    impl ResponseFinalized {
        ...
        ) -> Result<&ResponseFinalized, ErrorCode> {
            let resp = Response::mut_from_prefix_with_elems(response_buf, 4)?;
            resp.data_bytes.copy_from_slice(u32::from(err).as_bytes());
            Ok(resp.finalize(Status::ERROR))
        }
    }

    impl uDebug for ResponseFinalized {
        fn fmt<W: uWrite + ?Sized>(&self, f: &mut ufmt::Formatter<'_, W>) -> Result<(), W::Error> {
            // This says: "To format a `ResponseFinalized`, just use the
            // formatting logic for the inner `Response` object (self.0)."
            ufmt::uDebug::fmt(&self.0, f)
        }
    }
    ...
    mod test_response {
        ...
        fn debug() {
            ...
            assert_eq!(
                format!("{res:?}"),
                "Response { version: 1, checksum: 2, result: Status(3), data_len: 1284, extra: 6, data: [0x07, 0x08, ] }"
            );
        }
    }
}
...
mod test {
    ...
    fn test_hostcmd_req_build_fixed() {
        ...
        assert_eq!(req.data.as_bytes(), &[0_u8; 8]);
    ...
    fn test_hostcmd_req_build_fixed_3byte() {
        ...
        assert_eq!(req.data.as_bytes(), &[0_u8; 3]);
        req.data.a = 0x11;
        req.data.b = 0x22;
        req.data.c = 0x33;
        let req = req.finalize();
    ...
    #[test]
    fn test_hostcmd_req_build_fixed_buffer_overflow() {
        let mut buf = Aligned::<A4, _>([0x55_u8; 4]);
        assert_eq!(
            TestFixedReq::build_fixed(&mut buf).unwrap_err(),
            HOSTCMD_BUFFER_OVERFLOW
        );

        let mut buf = Aligned::<A4, _>([0x55_u8; size_of::<ReqBuilder<TestFixedReq3>>() - 1]);
        assert_eq!(
            TestFixedReq3::build_fixed(&mut buf).unwrap_err(),
            HOSTCMD_BUFFER_OVERFLOW,
        );

        let mut buf = Aligned::<A4, _>([0x55_u8; size_of::<ReqBuilder<TestFixedReq3>>()]);
        TestFixedReq3::build_fixed(&mut buf).unwrap();
    }
    ...
    fn test_response_from_error() {
        ...
        let response = ResponseFinalized::from_error(
            libhoth_error::FLASH_GENERIC_SFDP_UNSUPPORTED_HEADER_MAJOR_REV,
            &mut response_buf,
        )
        .unwrap();
    ...
    fn test_response_respond_ok() {
        ...
        assert_eq!(
            ...
            Some(libhoth_error::IO_GENERIC_ZEROCOPY_CONV_SIZE)
        );
    }
