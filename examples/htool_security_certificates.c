...
static int get_cert_v2_data(const struct htool_invocation* inv, uint32_t major_command,
                    uint32_t minor_command, uint8_t* cert, uint32_t cert_size, 
                    uint8_t response_storage_hdr[], uint32_t response_storage_hdr_size) {
  int status = -1;
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      int hoth_status = htool_exec_security_v2_cmd(
          ...
          SECURITY_V2_BUFFER_PARAM_WITH_VARIABLE_SIZE(response_storage_hdr, 
                                                      response_storage_hdr_size),
          response_params,
      ...
      fwrite(cert, cert_size, NUM_OF_CERTS_WRITTEN,
             output_ptr);
      break;
    }
...
int htool_get_attestation_pub_cert(const struct htool_invocation* inv) {
  ...
  return get_cert_v2_data(inv, HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_CERTIFICATES_MAJOR_COMMAND,
                    HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_ATTESTATION_PUB_CERT_MINOR_COMMAND,
                    cert, ATTESTATION_CERT_SIZE, response_storage_hdr, sizeof(response_storage_hdr));
}
...
int htool_get_alias_key_v0_cert(const struct htool_invocation* inv) {
  ...
  return get_cert_v2_data(inv, HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_CERTIFICATES_MAJOR_COMMAND,
                    ...
                    cert, ALIAS_KEY_V0_SIZE, response_storage_hdr, sizeof(response_storage_hdr));
}
...
int htool_get_alias_key_v1_cert(const struct htool_invocation* inv) {
  ...
  return get_cert_v2_data(inv, HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_CERTIFICATES_MAJOR_COMMAND,
                    HOTH_PRV_CMD_HOTH_SECURITY_V2_GET_ALIAS_KEY_MINOR_COMMAND,
                    cert, ALIAS_KEY_V1_SIZE, response_storage_hdr, sizeof(response_storage_hdr));
}
...
int htool_get_device_id_cert(const struct htool_invocation* inv) {
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      if (is_cert_output_file_provided) {
        fwrite(&certificate, sizeof(certificate),
               NUM_OF_CERTS_WRITTEN, cert_output_ptr);
      }
