...
struct libhoth_device* htool_libhoth_usb_device(void) {
  ...
  if (htool_get_param_string(htool_global_flags(), "retry_duration", &duration_str) ||
  ...
  if (htool_get_param_string(htool_global_flags(), "usb_retry_duration", &usb_duration_str) ||
      htool_get_param_string(htool_global_flags(), "usb_retry_delay", &usb_delay_str)) {
    return NULL;
  }
  ...
  if (usb_retry_duration_us != 0) {
    fprintf(stderr, "Warning: --usb_retry_duration is deprecated, use --retry_duration\n");
    retry_duration_us = usb_retry_duration_us;
    duration_str = usb_duration_str;
  }

  if (usb_retry_delay_us != 0) {
    fprintf(stderr, "Warning: --usb_retry_delay is deprecated, use --retry_delay\n");
