...
static int i2c_detect(struct libhoth_device *dev, const struct htool_invocation* inv)
{
    uint32_t bus;
    uint32_t start_addr;
    uint32_t end_addr;
    ...
    int ret = htool_exec_hostcmd(
        dev, EC_CMD_BOARD_SPECIFIC_BASE + EC_PRV_CMD_HOTH_I2C_DETECT, 0,
...
static int i2c_read(struct libhoth_device *dev, const struct htool_invocation* inv)
{
    uint32_t bus;
    uint32_t freq;
    uint32_t addr;
...
int htool_i2c_run(const struct htool_invocation* inv)
{
    ...
    if (strncmp(inv->cmd->verbs[1], I2C_DETECT_CMD_STR, sizeof(I2C_DETECT_CMD_STR)) == 0) {
        return i2c_detect(dev, inv);
    } else if (strncmp(inv->cmd->verbs[1], I2C_READ_CMD_STR, sizeof(I2C_READ_CMD_STR)) == 0) {
        return i2c_read(dev, inv);
    } else if (strncmp(inv->cmd->verbs[1], I2C_WRITE_CMD_STR, sizeof(I2C_WRITE_CMD_STR)) == 0) {
