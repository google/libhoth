...
static int htool_get_tokens_in_set(const struct htool_invocation* inv,
                                  uint32_t set_idx,
                                  uint8_t* data, uint32_t data_index,
  ...
  int num_written = append_index_to_file_name(
      token_output_file, sizeof(token_output_file), token_output_file_base,
      set_idx);
  ...
  if (data == NULL) {
    ...
    if (num_written < 0 || (size_t)num_written >= sizeof(signature_output_file)) {
    ...
    if (num_written < 0 || (size_t)num_written >= sizeof(boot_nonce_output_file)) {
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      if (token_output_ptr && boot_nonce_output_ptr && signature_output_ptr) {
        ...
        fwrite(tokens_param->value, tokens_param->size, NUM_OBJS_WRITTEN, token_output_ptr);
        fwrite(boot_nonce_param->value, boot_nonce_param->size, NUM_OBJS_WRITTEN, boot_nonce_output_ptr);
        fwrite(sig_param->value, sig_param->size, NUM_OBJS_WRITTEN, signature_output_ptr);
      } else {
        memcpy(data + data_index, tokens_param->value, tokens_param->size);
        *bytes_written += tokens_param->size;
        memcpy(data + data_index + *bytes_written, boot_nonce_param->value, boot_nonce_param->size);
        *bytes_written += boot_nonce_param->size;
        memcpy(data + data_index + *bytes_written, sig_param->value, sig_param->size);
...
static int htool_get_token_set_count(const struct htool_invocation* inv,
  ...
  if (data == NULL) {
    ...
    if (htool_get_param_string(inv, "token_count_boot_nonce_output", &boot_nonce_file) != 0) {
      goto cleanup;
    }
    ...
    if (htool_get_param_string(inv, "token_count_signature_output", &signature_file) != 0) {
      goto cleanup;
    }
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      uint8_t request_storage_hdr[HOTH_SECURITY_V2_REQUEST_SIZE(1) + sizeof(challenge)] = {};
      uint8_t response_storage_hdr[HOTH_SECURITY_V2_RESPONSE_SIZE(3) + sizeof(uint32_t) 
                              + sizeof(boot_nonce) + sizeof(signature)] = {};
      ...
      if (num_ids_ptr && boot_nonce_ptr && signature_ptr) {
        ...
        fwrite(&boot_nonce, sizeof(boot_nonce), NUM_OBJS_WRITTEN, boot_nonce_ptr);
        fwrite(&signature, sizeof(signature), NUM_OBJS_WRITTEN, signature_ptr);
      } else {
        memcpy(data + data_index, num_ids, sizeof(*num_ids));
        memcpy(data + data_index + sizeof(*num_ids), &boot_nonce, sizeof(boot_nonce));
        memcpy(data + data_index + sizeof(*num_ids) + sizeof(boot_nonce), &signature, sizeof(signature));
      }
      break;
    }
    // SECURITY_V3 not supported.
...
static int htool_get_token_set_info(const struct htool_invocation* inv,
  ...
  if (data == NULL) {
    ...
    if (htool_get_param_string(inv, "token_set_info", &token_set_info_file_base) !=
        0) {
      return -1;
    }
    char token_set_info_file[MAX_STRING_BUFFER_SIZE];
    int num_written =
      append_index_to_file_name(token_set_info_file, sizeof(token_set_info_file),
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      uint8_t request_storage_hdr[HOTH_SECURITY_V2_REQUEST_SIZE(2) + sizeof(set_idx) + sizeof(challenge)] = {};
      uint8_t response_storage_hdr[HOTH_SECURITY_V2_RESPONSE_SIZE(3) + sizeof(info) 
                                + sizeof(boot_nonce) + sizeof(signature)] = {};
...
static int htool_fetch_single_attestation_file(const struct htool_invocation* inv, const char* filename){
  int status = -1;
  struct libhoth_device* dev = htool_libhoth_device();
  if (!dev) {
    return -1;
  ...
  data_index += sizeof(num_ids) + sizeof(struct boot_nonce) + sizeof(struct detached_challenge_response_signature);
  ...
  switch (sv) {
    case LIBHOTH_SECURITY_V2: {
      ...
      for (int i = 0; i < num_ids; i++) {
        ...
        // Get Tokens in Set
        status = htool_get_tokens_in_set(inv, i, data, data_index, &bytes_written);
        if (status != 0) {
          return status;
        }
...
int htool_fetch_attestation(const struct htool_invocation* inv) {
  ...
  // Get the attestation_file flag, if the flag is not provided retrieve the individual
  // attestation fields.
  if (strlen(attestation_file) == 0) {
    //Get Token Set Count
    uint32_t num_ids = 0;
    status = htool_get_token_set_count(inv, &num_ids, /*data=*/NULL, /*data_index=*/0);
    if (status != 0) {
      return status;
    }
    ...
    for (int i = 0; i < num_ids; i++) {
      //Get Tokens in Set
      status = htool_get_tokens_in_set(inv, i, /*data=*/NULL, /*data_index=*/0, /*bytes_written=*/ NULL);
      if (status != 0) {
        return status;
      }

      //Get Token Set Info
      status = htool_get_token_set_info(inv, i, /*data=*/NULL, /*data_index=*/0);
      if (status != 0) {
        return status;
      }
    }
