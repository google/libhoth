...
#define EC_PRV_CMD_HAVEN_CONT_STORAGE 0x0015
#define CONT_STORAGE_SIZE_MAX 128
#define CONT_STORAGE_SIZE 64

enum controlled_storage_op {
  CONT_STORAGE_READ = 0,
  CONT_STORAGE_WRITE = 1,
  CONT_STORAGE_DELETE = 2,
};

// Serves as the response to CONT_STORAGE_READ as well as payload to CONT_STORAGE_WRITE.
struct ec_payload_controlled_storage {
  uint8_t data[CONT_STORAGE_SIZE];
} __attribute__((packed));

struct ec_request_controlled_storage {
  /* The operation is one of controlled_storage_op. */
  uint32_t operation;
  /* Choose which (0-5) slot to operate on. */
  uint32_t slot;
  struct ec_payload_controlled_storage payload;
} __attribute__((packed));


int libhoth_controlled_storage_read(struct libhoth_device* dev, uint32_t slot, 
        struct ec_payload_controlled_storage* payload, size_t* payload_len);
