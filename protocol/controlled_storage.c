...
int libhoth_controlled_storage_read(
  ...
  struct ec_request_controlled_storage req = {};

  req.operation = CONT_STORAGE_READ;
  req.slot = slot;
  return libhoth_hostcmd_exec(
      dev, EC_CMD_BOARD_SPECIFIC_BASE + EC_PRV_CMD_HAVEN_CONT_STORAGE,
...
int libhoth_controlled_storage_write(struct libhoth_device* dev, uint32_t slot,
  ...
  req.operation = CONT_STORAGE_WRITE;
  req.slot = slot;
  memcpy(req.payload.data, data, len);
  return libhoth_hostcmd_exec(
      dev, EC_CMD_BOARD_SPECIFIC_BASE + EC_PRV_CMD_HAVEN_CONT_STORAGE,
      /*version=*/0, &req,
      sizeof(req) - sizeof(struct ec_payload_controlled_storage) + len, NULL, 0,
      NULL);
}
...
int libhoth_controlled_storage_delete(struct libhoth_device* dev,
  ...
  req.operation = CONT_STORAGE_DELETE;
  req.slot = slot;
  return libhoth_hostcmd_exec(
      dev, EC_CMD_BOARD_SPECIFIC_BASE + EC_PRV_CMD_HAVEN_CONT_STORAGE,
