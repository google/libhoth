...
struct image_descriptor* libhoth_find_image_descriptor(uint8_t* image,
                                                       size_t len) {
  for (size_t off = 0; off + sizeof(int64_t) - 1 < len;
    ...
    if (magic_candidate == TITAN_IMAGE_DESCRIPTOR_MAGIC) {
      return (struct image_descriptor*)(image + off);
    }
  }
  return NULL;
}
...
bool libhoth_payload_info(uint8_t* image, size_t len,
  ...
  struct image_descriptor* descr = libhoth_find_image_descriptor(image, len);
  if (descr == NULL) {
    return false;
  }
